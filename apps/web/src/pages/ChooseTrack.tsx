    ...
    return (
        <div className='bg-gray-50'>
            <div className='text-2xl pt-10 sm:w-3/4 w-11/12 m-auto'>
                <div>
                    <p className='text-center font-semibold mb-5'>which track are you interested in?</p>
                    <div className='grid md:grid-cols-4 grid-cols-2 gap-5'>
                        {tracks.map(t => {
                            const Icon = t.icon
                            return (
                                <div className={`font-semibold flex flex-col gap-2 items-center ${selected === t.name ? 'bg-IEEEorange text-white' : 'text-IEEEorange bg-white'} cursor-pointer text-center rounded-md p-5 py-10 shadow-md`} onClick={() => setSelected(t.name)}>
                                    <Icon />
                                    <p>{t.name}</p>
                                </div>
                            )
                ...
                <div className='sm:mt-24 mt-10'>
                    <p className='text-center font-semibold mb-5'>What is your current level in Programming Basics</p>
                        ...
                        <div className='flex justify-between md:w-3/4 m-auto mb-10'>
                            {
                                levels.map(l => (
                                    <div className='flex gap-2 items-center sm:text-2xl text-lg ' >
                                        <input type='radio' name='level' className='w-5 h-5 peer' /> {/* checked:accent-IEEEorange   */}
